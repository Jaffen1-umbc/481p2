...
MESSAGE_QUEUE = []	#the global array of messages incoming from servers awaiting to be read from games
...
class TTT_Game:
	...
	def game_thread(self):
		...
		#GET RESPONSE
		cmd_line_args = self.await_a_message()
		...
		while endgame_status == UNUSED_MARK:
			...
			#if server turn
			elif self.get_turn() == SERVER_MARK:
...
def get_message_for(addr):
	for a, v in MESSAGE_QUEUE:
		if a == addr:
			MESSAGE_QUEUE.remove((a,v))
			return v
